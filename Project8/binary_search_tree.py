...
class BST(object):  # A binary search tree of data elements
    ...
    def recursiveAddHelper(self, root,key):
        if root is None:
            return key
        if key < root.key:
            root.left = self.recursiveAddHelper(root.left,key)
        else:
            root.right = self.recursiveAddHelper(root.right,key)
        
        return root
    
    def recursiveAdd(self,key):
        message = self.recursiveAddHelper(self.__root,key)
